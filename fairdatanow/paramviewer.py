...
# %% auto #0
__all__ = ['filters', 'SaveLogin', 'login', 'to_iframe', 'DataViewer']

# %% ../notebooks/10_exploring-your-remote-data-in-breeze.ipynb #672efd20-e8fa-4c44-9966-9b85e843f0ac
import panel as pn
import param
import pandas as pd
import nc_py_api
...
class SaveLogin(Viewer):
    url = param.String(default="", doc="URL of your Nextcloud server")
    user = param.String(default="", doc="Username/email of your Nextcloud account")
    password = param.String(default="", doc="Password of your Nextcloud account")
    
    alert = pn.pane.Alert("Fill in all fields before pressing Save log in.", alert_type="info")

    def __init__(self, **params):
        super().__init__(**params)
    
    def __panel__(self):
        url_input = pn.widgets.TextInput.from_param(self.param.url, label='Nexcloud URL', placeholder='https://example.com/project-name')
        username_input = pn.widgets.TextInput.from_param(self.param.user, label='Account name or email', placeholder='Account name or email')
        password_input = pn.widgets.PasswordInput.from_param(self.param.password, label='Password', placeholder='Password')

        button = pn.widgets.Button(label='Save log in', on_click=self.test_info, color='primary')
    ...
    def test_info(self, event, sep='\t',): 
        ...
        # check
        if not m:
            self.alert.object = f'Invalid nextcloud url. Needs to be formatted as: https://<my-server.com>/<folder>'
            self.alert.alert_type = "danger"
            return
        
        if not password:
            self.alert.object = f'Please provide a password to succesfully save your login.'
            self.alert.alert_type = "danger"
            return
        ...
        if test_ok:  
            ...
            self.alert.object = f'Saved user name and password for: {nextcloud_url}'
            self.alert.alert_type = "success"

        if not test_ok: 
            # do not save 
            print(f'Did not save login due to incorrect credentials for nextcloud server: {nextcloud_url}')  
            self.alert.object = f'Did not save login due to incorrect credentials for nextcloud server: {nextcloud_url}'
            self.alert.alert_type = "danger"
